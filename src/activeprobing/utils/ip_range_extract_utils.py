...
from src.activeprobing.configs.path_config import (
    RESOURCES_JSONS_DIR_PATH,
    RESOURCES_IP_RANGES_DIR_PATH,
)

aliyun_ips_json = RESOURCES_JSONS_DIR_PATH / "aliyun_ips_domain_binding_ips.json"
tencent_cloud_ips_json = (
    RESOURCES_JSONS_DIR_PATH / "tencent_cloud_ips_domain_binding_ips.json"
)
...
def extract_amazon_ips(json_path: str, output_file: str):
    ...
    with ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(process_chunk, chunk, output_file, lock) for chunk in chunks
        ]
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Processing Amazon prefixes"
        ):
...
def extract_all_sdbs_ips():
    ...
    extract_sdb_ips(
        tencent_cloud_ips_json, tencent_cloud_ips_save_path, "tencent_cloud"
    )
