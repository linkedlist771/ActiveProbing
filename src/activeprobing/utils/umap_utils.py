...
import datetime
from src.activeprobing.configs.ports_config import NORMAL_SERVICE_PORTS, NORMAL_SCAN_TIMEOUT, FULL_SCAN_TIMEOUT
...
class NmapManger(object):
    ...
    def get_scan_command(self):
        if self.scan_type == ScanType.NORMAL:
            return partial(subprocess.run,
                           ['nmap', '-p', NORMAL_SERVICE_PORTS, '-T4', self.ip], capture_output=True, text=True, timeout=NORMAL_SCAN_TIMEOUT)
        elif self.scan_type == ScanType.FULL:
            return partial(subprocess.run,
                           ['nmap', '-p-', '-T4', self.ip], capture_output=True, text=True, timeout=FULL_SCAN_TIMEOUT)
        else:
