    ...
    async def scrape_domain_binding_url(self, url, max_retries=3, base_delay=1):
        ...
        for i in range(max_retries):
            try:
                ...
                elements_text = [
                    element.text(strip=True).replace(":", "") for element in elements
                ]
...
async def scrapy_sdb_domain_binding_ips():
    ...
    async def process_ip_json(ip_json):
        ...
        logger.info(
            f"保存了 {len(domain_binding_ips)} 条域名绑定 IP 记录到 {save_path}"
        )
    ...
    try:
        results = await asyncio.gather(
            *[process_ip_json(ip_json) for ip_json in ips_jsons]
        )
